...
namespace Masterv2
{
    public class JointCheckerList : GH_Component
    {
        /// <summary>
        /// Initializes a new instance of the JointCheckerList class.
        /// </summary>
        public JointCheckerList()
          : base("JointCheckerList", "Nickname",
              "Description",
              "Master", "Check")
        {
        }
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("TrussDimensions", "Tdim", "Dimensions of the truss [h_top, b_top, h_truss, b_truss, h_bottom, b_bottom] [cm]", GH_ParamAccess.list);
            pManager.AddTextParameter("Material", "mat", "Material of the beams", GH_ParamAccess.item);
            pManager.AddNumberParameter("DowelDiameter", "dd", "Diameter of the dowels", GH_ParamAccess.list);
        }

        protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager)
        {
            pManager.AddGenericParameter("test1", "", "", GH_ParamAccess.list);
            pManager.AddGenericParameter("test2", "", "", GH_ParamAccess.tree);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            var trussdim = new List<double>();
            var material = "";
            var dlist = new List<double>();

            DA.GetDataList(0, jointforces);
            DA.GetDataList(1, trussdim);
            DA.GetData(2, ref material);
            DA.GetDataList(3,  dlist);
            ...
            var pointsmain = new List<List<Point3d>>();
            ...
            //Loop for each of the diameters of the dowels
            foreach (var d in dlist)
            {
                ...
                result.Add($"Dowel diameter: {d} mm");
                for (int a = 0; a < 3; a++)
                {
            ...
            */
            var sortedpointsNlist = new List<List<Point3d>>();
            foreach (var pointslist in pointsmain)
            {
                var chunk = pointslist.Count / 3;
                List<Point3d> points1 = pointslist.GetRange(0, chunk);
                List<Point3d> points2 = pointslist.GetRange(chunk, chunk);
                List<Point3d> points3 = pointslist.GetRange(2 * chunk, chunk);

                var sortedpoints = points1.OrderBy(pt => pt.X).ToList();
                sortedpointsNlist.Add(sortedpoints);
            }

            var pointtree = GHTreeMaker.NestedList(pointsmain);
            var sortedpointstree = GHTreeMaker.NestedList(sortedpointsNlist);
            //Output
            DA.SetDataList(0, result);
            DA.SetDataTree(1, sortedpointstree);
            DA.SetDataList(2, DowelsSteelplates.DowelCoordinates(10, 20, 10, 12)); //pointsmain[0]
        ...
        {
            get { return new Guid("0FAF0864-E8B4-4190-B9D6-D600DC33987A"); }
        }
    }
}